...
/**
 * The {@code DragSource} is the entity responsible for the initiation of the
 ...
 * {@code Component}, or application specific object associated with a
 * {@code Component} instance in the GUI. [implementation dependent]
 * <LI>Some other arbitrary association. [implementation dependent]
 * </UL>
 * <p>
 * Once the {@code DragSource} is obtained, a {@code DragGestureRecognizer}
 * should also be obtained to associate the {@code DragSource} with a particular
 * {@code Component}.
 * <p>
 * The initial interpretation of the user's gesture, and the subsequent starting
 * of the drag operation are the responsibility of the implementing
 * {@code Component}, which is usually implemented by a
 * {@code DragGestureRecognizer}.
 * <p>
 * When a drag gesture occurs, the {@code DragSource}'s startDrag() method shall
 * be invoked in order to cause processing of the user's navigational gestures
 * and delivery of Drag and Drop protocol notifications. A {@code DragSource}
 * shall only permit a single Drag and Drop operation to be current at any one
 * time, and shall reject any further startDrag() requests by throwing an
 * {@code IllegalDnDOperationException} until such time as the extant operation
 * is complete.
 * <p>
 * The startDrag() method invokes the createDragSourceContext() method to
 * instantiate an appropriate {@code DragSourceContext} and associate the
 * {@code DragSourceContextPeer} with that.
 * <p>
 * If the Drag and Drop System is unable to initiate a drag operation for some
 * reason, the startDrag() method throws a
 * {@code java.awt.dnd.InvalidDnDOperationException} to signal such a condition.
 * Typically this exception is thrown when the underlying platform system is
 * either not in a state to initiate a drag, or the parameters specified are
 * invalid.
 * <p>
 * Note that during the drag, the set of operations exposed by the source at the
 * start of the drag operation may not change until the operation is complete.
 * The operation(s) are constant for the duration of the operation with respect
 * to the {@code DragSource}.
 *
 * @since 1.2
 */
public class DragSource implements Serializable {
    ...
    /**
     * The default {@code Cursor} to use with a move operation indicating that a
     * drop is currently allowed. {@code null} if
     * {@code GraphicsEnvironment.isHeadless()} returns {@code true}.
     *
     * @see java.awt.GraphicsEnvironment#isHeadless
    ...
    /**
     * The default {@code Cursor} to use with a link operation indicating that a
     * drop is currently allowed. {@code null} if
     * {@code GraphicsEnvironment.isHeadless()} returns {@code true}.
     *
     * @see java.awt.GraphicsEnvironment#isHeadless
    ...
    /**
     * The default {@code Cursor} to use with a copy operation indicating that a
     * drop is currently not allowed. {@code null} if
     * {@code GraphicsEnvironment.isHeadless()} returns {@code true}.
     *
     * @see java.awt.GraphicsEnvironment#isHeadless
     */
    ...
    /**
     * Gets the {@code DragSource} object associated with the underlying
     * platform.
     *
    ...
    /**
     * Start a drag, given the {@code DragGestureEvent} that initiated the drag,
     * the initial {@code Cursor} to use, the {@code Image} to drag, the offset
     * of the {@code Image} origin from the hotspot of the {@code Cursor} at the
     ...
     * the initial {@code Cursor} to use, the {@code Image} to drag, the offset
     * of the {@code Image} origin from the hotspot of the {@code Cursor} at the
     * instant of the trigger, the subject data of the drag, and the
     ...
     * @param  dragOffset the offset of the {@code Image} origin from the
     ...
     * operation.
     * <p>
     * To incorporate a new {@code DragSourceContext} subclass, subclass
     * {@code DragSource} and override this method.
     * <p>
     * If {@code dragImage} is {@code null}, no image is used to represent the
     * drag over feedback for this drag operation, but
     * {@code NullPointerException} is not thrown.
     * <p>
     * If {@code dsl} is {@code null}, no drag source listener is registered
     * with the created {@code DragSourceContext}, but
     * {@code NullPointerException} is not thrown.
     *
     * @param  dgl The {@code DragGestureEvent} that triggered the drag
     * @param  dragCursor The initial {@code Cursor} for this drag operation or
     *         {@code null} for the default cursor handling; see <a
     *         href="DragSourceContext.html#defaultCursor">DragSourceContext</a>
     *         class for more details on the cursor handling mechanism during
     ...
     * @throws NullPointerException if {@code dragImage} is not {@code null} and
     *         {@code imageOffset} is {@code null}
    ...
    /**
     * This method returns the {@code FlavorMap} for this {@code DragSource}.
     *
     ...
     * abstract subclass of {@code DragGestureRecognizer}, and sets the
     * specified {@code Component} and {@code DragGestureListener} on the newly
     * created object.
     *
     ...
     * @return the new {@code DragGestureRecognizer} or {@code null} if the
     *         {@code Toolkit.createDragGestureRecognizer} method has no
     *         implementation available for the requested
     *         {@code DragGestureRecognizer} subclass and returns {@code null}
     */
     ...
     * {@code DragSource}, and sets the specified {@code Component} and
     * {@code DragGestureListener} on the newly created object.
     * <p>
     * For this {@code DragSource} the default is
     * {@code MouseDragGestureRecognizer}.
     *
    ...
    /**
     * Adds the specified {@code DragSourceListener} to this {@code DragSource}
     * to receive drag source events during drag operations initiated with this
     * {@code DragSource}. If a {@code null} listener is specified, no action is
     ...
     * {@code DragSource}. If a {@code null} listener is specified, no action is
     * taken and no exception is thrown. If the listener specified by the
     * argument was not previously added to this {@code DragSource}, no action
    ...
    /**
     * Gets all the {@code DragSourceListener}s registered with this
     * {@code DragSource}.
     *
     * @return all of this {@code DragSource}'s {@code DragSourceListener}s or
     *         an empty array if no such listeners are currently registered
     * @see #addDragSourceListener
     * @see #removeDragSourceListener
     * @since 1.4
     */
     ...
     * {@code DragSource} to receive drag motion events during drag operations
     * initiated with this {@code DragSource}. If a {@code null} listener is
     * specified, no action is taken and no exception is thrown.
     *
     ...
     *         {@code DragSourceMotionListener}s or an empty array if no such
    ...
    /**
     * This method calls {@code dragEnter} on the {@code DragSourceListener}s
     * registered with this {@code DragSource}, and passes them the specified
    ...
    /**
     * This method calls {@code dragOver} on the {@code DragSourceListener}s
     * registered with this {@code DragSource}, and passes them the specified
     * {@code DragSourceDragEvent}.
     *
     * @param  dsde the {@code DragSourceDragEvent}
     ...
     * {@code DragSourceListener}s registered with this {@code DragSource}, and
     * passes them the specified {@code DragSourceDragEvent}.
     *
     * @param  dsde the {@code DragSourceDragEvent}
     */
    ...
    /**
     * This method calls {@code dragExit} on the {@code DragSourceListener}s
     * registered with this {@code DragSource}, and passes them the specified
    ...
    /**
     * This method calls {@code dragDropEnd} on the {@code DragSourceListener}s
     * registered with this {@code DragSource}, and passes them the specified
    ...
    /**
     * Serializes this {@code DragSource}. This method first performs default
     * serialization. Next, it writes out this object's {@code FlavorMap} if and
     * only if it can be serialized. If not, {@code null} is written instead.
     * Next, it writes out {@code Serializable} listeners registered with this
     * object. Listeners are written in a {@code null}-terminated sequence of 0
     * or more pairs. The pair consists of a {@code String} and an
     ...
     * @serialData Either a {@code FlavorMap} instance, or {@code null},
     *         followed by a {@code null}-terminated sequence of 0 or more
     *         pairs; the pair consists of a {@code String} and an
     *         {@code Object}; the {@code String} indicates the type of the
     *         {@code Object} and is one of the following:
     *         <ul>
     *         <li>{@code dragSourceListenerK} indicating a
    ...
    /**
     * Deserializes this {@code DragSource}. This method first performs default
     * deserialization. Next, this object's {@code FlavorMap} is deserialized by
     * using the next object in the stream. If the resulting {@code FlavorMap}
     * is {@code null}, this object's {@code FlavorMap} is set to the default
     * FlavorMap for this thread's {@code ClassLoader}. Next, this object's
     * listeners are deserialized by reading a {@code null}-terminated sequence
     * of 0 or more key/value pairs from the stream:
     * <ul>
    ...
    /**
     * Returns the drag gesture motion threshold. The drag gesture motion
     * threshold defines the recommended behavior for
     * {@link MouseDragGestureRecognizer}s.
     * <p>
     * If the system property {@code awt.dnd.drag.threshold} is set to a
     * positive integer, this method returns the value of the system property;
     ...
     * that property; otherwise this method returns some default value. The
     * pertinent desktop property can be queried using
     * {@code
